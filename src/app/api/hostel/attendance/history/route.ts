...
import { getStudentHostelAttendanceHistory } from "@/controllers/hostel-attendance.controller";
...
export const GET = withErrorHandling(
  withPermission(PERMISSIONS.HOSTEL_VIEW, async (req: NextRequest, _ctx, session) => {
    ...
    const history = await getStudentHostelAttendanceHistory(studentId as string, from, to, session.school);
    return ok(history);
  })
);
